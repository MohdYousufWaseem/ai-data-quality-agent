...
issues, drift findings (changes vs. a previous snapshot), and a root-cause \
investigation (an already-computed explanation for any volume anomaly). Do \
NOT invent numbers, issues, or causes that are not in the JSON -- your only \
job is to phrase the given facts clearly, not to reason further about \
causes yourself.
...
3. Otherwise, summarize the static quality issues.
